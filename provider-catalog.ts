...
export const NOUS_PORTAL_DEFAULT_MODEL_ID =
  "inclusionai/ling-3.0-flash-sante:free";
export const NOUS_PORTAL_DEFAULT_MODEL_REF = `nous-portal/${NOUS_PORTAL_DEFAULT_MODEL_ID}`;
const FALLBACK_MODELS: readonly ModelDefinitionConfig[] = [
  {
  ...
  for (const row of rows) {
    ...
    models.set(id, {
      ...
      cost: {
        ...
        cacheRead:
          readPrice(pricing?.input_cache_read) ?? seed?.cost.cacheRead ?? 0,
        cacheWrite:
          readPrice(pricing?.input_cache_write) ?? seed?.cost.cacheWrite ?? 0,
      },
...
export const NOUS_PORTAL_MODEL_DISCOVERY: OpenAICompatibleModelDiscoveryOptions =
  {
    endpointUrl: {
